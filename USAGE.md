# DoScroll Documentation

DoScroll is a Python library for maintaining an efficient local copy of Todoist tasks with time tracking, state management, and workflow automation.

## Installation

```bash
# Clone or place the DoScroll directory in your project
cd /path/to/your/project
import src as doscroll
```
...
doscroll.todoist_sync.sync_tasks(API_TOKEN)

# Start working on a task
doscroll.task_manager.start_work_on_task("task_id_123", API_TOKEN)

# Stop working
doscroll.task_manager.stop_work_on_task("task_id_123")

# Mark task as complete
doscroll.task_manager.complete_task("task_id_123", API_TOKEN)
```
...
```python
doscroll.task_manager.start_work_on_task("1234567890", API_TOKEN)
...
```python
doscroll.task_manager.stop_work_on_task("1234567890")
# Returns: True on success, False on failure
```
...
```python
doscroll.task_manager.complete_task("1234567890", API_TOKEN)
...
```python
summary = doscroll.task_manager.get_task_summary("1234567890")
# Returns: {
#     "task": {...},
...
```python
summary = doscroll.task_manager.get_active_task_summary()
...
```python
all_tasks = doscroll.task_manager.get_all_tasks()
incomplete_tasks = doscroll.task_manager.get_all_tasks(include_completed=False)
completed_tasks = doscroll.task_manager.get_all_tasks(include_completed=True)
```
...
```python
incomplete = doscroll.task_manager.get_incomplete_tasks()
```
...
```python
completed = doscroll.task_manager.get_completed_tasks()
```
...
```python
doscroll.time_tracking.start_task("1234567890")
...
```python
doscroll.time_tracking.stop_task("1234567890")
...
```python
doscroll.time_tracking.stop_all_tasks()
```
...
```python
total_seconds = doscroll.time_tracking.get_total_time_for_task("1234567890")
```
...
```python
entries = doscroll.time_tracking.get_time_entries_for_task("1234567890")
```
...
```python
all_entries = doscroll.time_tracking.get_all_time_entries()
```
...
```python
duration = doscroll.time_tracking.get_current_session_duration()
```
...
```python
# Export all tasks
doscroll.time_tracking.export_time_report_csv()

# Export specific task
doscroll.time_tracking.export_time_report_csv(task_id="1234567890")

# Export to custom path
doscroll.time_tracking.export_time_report_csv(filepath="/path/to/report.csv")
```
...
```python
tasks = doscroll.todoist_sync.fetch_tasks_from_api(API_TOKEN)
```
...
```python
doscroll.todoist_sync.save_tasks_locally(tasks)
```
...
```python
tasks = doscroll.todoist_sync.load_tasks_locally()
```
...
```python
doscroll.todoist_sync.mark_complete_remote("1234567890", API_TOKEN)
# Returns: True on success, False on failure
```
...
```python
doscroll.todoist_sync.mark_complete_local("1234567890")
```
...
```python
is_valid = doscroll.todoist_sync.get_api_status(API_TOKEN)
```
...
```python
task = doscroll.task_state.get_active_task()
# Returns: dict or None if no active task
```
...
```python
task_id = doscroll.task_state.get_active_task_id()
# Returns: str or None
```
...
```python
is_active = doscroll.task_state.is_task_active("1234567890")
```
...
```python
doscroll.task_state.clear_active_task()
```
...
```python
task = doscroll.task_state.get_task_by_id("1234567890")
```
...
```python
exists = doscroll.task_state.task_exists("1234567890")
```
...
doscroll.background_sync.start_background_sync(API_TOKEN)

# Sync every minute
doscroll.background_sync.start_background_sync(API_TOKEN, interval_seconds=60)
```
...
```python
doscroll.background_sync.stop_background_sync()
```
...
```python
running = doscroll.background_sync.is_sync_running()
```
...
```python
doscroll.background_sync.set_sync_interval(120)  # 2 minutes
```
...
```python
doscroll.background_sync.trigger_sync_now()
```
...
```python
# Get file paths
doscroll.storage.get_tasks_filepath()        # data/tasks.json
doscroll.storage.get_time_tracking_filepath() # data/time_tracking.json
doscroll.storage.get_state_filepath()        # data/state.json
doscroll.storage.get_exports_dir()           # exports/
```
...
```python
# Tasks
doscroll.storage.save_tasks(tasks_list)
doscroll.storage.load_tasks()

# Time entries
doscroll.storage.save_time_entries(entries_list)
doscroll.storage.load_time_entries()

# State
doscroll.storage.save_state(state_dict)
doscroll.storage.load_state()
```
...
doscroll.background_sync.start_background_sync(API_TOKEN, 300)

# 3. List incomplete tasks
tasks = doscroll.task_manager.get_incomplete_tasks()
for task in tasks:
    print(f"{task['id']}: {task['content']}")

# 4. Start working on a task
task_id = tasks[0]['id']
doscroll.task_manager.start_work_on_task(task_id, API_TOKEN)

# 5. Check what we're working on
active = doscroll.task_manager.get_active_task_summary()
print(f"Working on: {active['task']['content']}")

# 6. After some time, stop and export report
doscroll.task_manager.stop_work_on_task(task_id)
doscroll.time_tracking.export_time_report_csv(task_id)

# 7. Mark task as complete
doscroll.task_manager.complete_task(task_id, API_TOKEN)
